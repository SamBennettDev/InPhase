...
test("dashboard shows pairing data, safe device names and a usable QR image", async ({
  ...
  await expect(page.locator("#play-url")).toHaveText(
    "https://gaming-pc.local/",
  );
  ...
  await expect(page.getByAltText("Scan to pair this device")).toHaveJSProperty(
    "naturalWidth",
    120,
  );
  expect(errors).toEqual([]);
});
test("failed dashboard actions are visible and can be retried", async ({
  page,
}) => {
...
test("library search and settings preserve selected quality and game", async ({
  page,
}, info) => {
  const errors: string[] = [];
  page.on("pageerror", (e) => errors.push(e.message));
  ...
  await openPlayer(page);
  await expect(
    page.getByRole("button", { name: "Stream desktop", exact: true }),
  ).toBeEnabled();
  ...
  await page
    .getByRole("button", { name: "Stream settings", exact: true })
    .click();
  ...
  await page
    .getByRole("button", { name: "Select Hades II", exact: true })
    .click();
  await expect(page.locator("#picklabel")).toHaveText("Hades II");
  await expect(page.locator("#stream-summary")).toContainText(
    "1440p · 120 fps",
  );
  ...
  const settings = await page.evaluate(() =>
    JSON.parse(localStorage.getItem("inphase.settings")!),
  );
  expect(settings).toMatchObject({
    height: 1440,
    fps: 120,
    streamTarget: { type: "game", id: "0" },
  });
  expect(errors).toEqual([]);
});
test("busy host prevents starting a second session", async ({ page }) => {
  ...
  await openPlayer(page);
  await expect(
    page.getByRole("button", { name: "PC in use", exact: true }),
  ).toBeDisabled();
  await expect(page.locator("#home-help")).toContainText("Another device");
});
test("library failure still permits desktop play and offers retry", async ({
  page,
}) => {
...
test("mobile layout fits and settings dialog supports keyboard dismissal", async ({
  page,
}, info) => {
  ...
  await page.screenshot({
    path: info.outputPath("library-mobile.png"),
    fullPage: true,
  });
  await page
    .getByRole("button", { name: "Stream settings", exact: true })
  ...
  await expect(
    page.getByRole("button", { name: "Stream settings", exact: true }),
  ).toBeFocused();
  await page.goto("/");
  await expect(page.locator("#pin")).toHaveText("482916");
  await expect
