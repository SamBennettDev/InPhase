...
impl Datagrams<'_> {
    ...
    pub fn send(&mut self, data: Bytes, drop: bool) -> Result<(), SendDatagramError> {
        ...
        if drop {
            ...
            {
                // INPHASE VENDOR PATCH: `pop_front` already decrements
                // `payload_bytes`; subtracting the same datagram again here
                // underflows the counter, wraps it, empties the queue, and
                // the `expect` below turns a buffer overflow into a panic
                // that poisons the connection (live: "payload_bytes
                // desynchronized", nine endpoint-wedging occurrences under
                // loss). Drop the duplicate bookkeeping and resync instead
                // of panicking if the accounting ever drifts anyway.
                let Some(prev) = self.conn.datagrams.outgoing.pop_front() else {
                    self.conn.datagrams.outgoing.payload_bytes = 0;
                    break;
                };
